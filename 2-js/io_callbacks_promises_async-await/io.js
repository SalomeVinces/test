/* 
    ...
    *NodeJS allows access to its processess via process obj
    *This can allow the process of IO
    *We can dothis by utilizing process.stdin and process.stdout
*/
//? Import Process
...
/* 
    ...
    *Fx that is based as an argument to another fx or method
...
const myFunction = (callbackFx) => {
    ...
    callbackFx("Hello World")
}

// ? Anonumous Arrow function
// myFunction((param) => {
//     console.log(param);
...
/* ? Promises
        - They are like a box, they must unpacked in order to received the value they contaim
        - Where a value may be pending, resolved, or rejection
...
async function handlePromise () {
    // In order to have access to await the function must have async
...
async function start (question) {
    ...
    try {
        ...
        if(response.lenght === 0){
            ...
            //Since we want to create a new type of `loop`, we'll use recursion
        }

        if(response === "y"){
            console.log("yayy, here's some pie");
        } else if (response === "n"){
